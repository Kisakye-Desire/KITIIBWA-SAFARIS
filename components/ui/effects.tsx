...
export function AnimatedCard({ children, className = '', delay = 0, floating = true }: { children: React.ReactNode; className?: string; delay?: number; floating?: boolean }) {
  const animationClass = floating ? 'animate-fade-in-up-float' : 'animate-fade-in-up'
  return (
    <div
      className={`card-hover rounded-lg p-6 bg-card border border-border ${animationClass} ${className}`}
      style={{
...
export function FloatingCard({ children, className = '', delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) {
  return (
    <div
      className={`rounded-lg p-6 bg-card border border-border card-hover animate-fade-in-up-float ${className}`}
      style={{
        animationDelay: `${delay * 0.1}s`,
      }}
    >
